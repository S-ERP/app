...
| # | Concepto | Activo | ¿Se usa acá? | Nota |
|---|---|:---:|---|---|
...
| 72 | Inmutabilidad | ⚠️ | Parcial / No | El proyecto **muta** `this.state` directo en muchos lugares (ej. `this.state.key_marca = x`) en vez de copiar |
| 73 | Refs vs State | ➖ | — | Conceptual |
| 74 | Props vs State | ➖ | — | Conceptual |
| 75 | Pure Components | ❌ | No | `React.PureComponent`: 0 archivos |
| 76 | Optimización | ⚠️ | Parcial | Poco `useMemo`/`memo`, la mayoría recalcula siempre |
| 77 | Rendering | ✅ | Sí | |
...
| 92 | Context + Reducer | ⚠️ | Parcial | Hay Context; `useReducer` no se usa |
...
| 96 | Performance | ⚠️ | Parcial | Sin profiling sistemático |
| 97 | Profiling | ❌ | No evidencia | |
| 98 | React DevTools | ➖ | — | Herramienta externa, no es parte del código |

## Lo que faltaba en la lista original

Esto es lo más importante que faltaba — y varios son cosas **muy** presentes en este proyecto específico:

| # | Concepto | Activo | ¿Se usa acá? | Nota |
|---|---|:---:|---|---|
| 99 | Higher-Order Components (HOC) | ✅ | **Sí, muchísimo** | 422 archivos usan `connect(...)` (patrón de `servisofts-page`, no Redux) — es el patrón dominante del proyecto, más que Context o Hooks |
| 100 | React Native / React Native Web | ✅ | **Sí, es la base de todo** | 520 archivos importan de `react-native`. El proyecto no es "React DOM puro": corre sobre React Native Web (por eso `SView`, `onPress` en vez de `onClick`, etc.) |
| 101 | `FlatList` / listas virtualizadas | ✅ | Sí | 51 archivos (ej. la lista de items del carrito en `PopupCarrito.tsx`) |
| 102 | Animated API (`Animated.*`) | ✅ | Sí | 60 archivos |
| 103 | `TouchableOpacity` | ✅ | Sí | 19 archivos |
| 104 | `React.forwardRef` | ✅ | Sí | 17 archivos |
| 105 | `defaultProps` | ✅ | Sí | 9 archivos |
| 106 | `createRoot` (API de montaje React 18) | ✅ | Sí | `src/index.js` — el proyecto ya usa React 18.2.0 |
| 107 | `React.StrictMode` | ❌ | No | 0 archivos — no está activado |
| 108 | PropTypes | ❌ | No | 0 archivos (usan TypeScript/Flow en su lugar, no siempre) |
| 109 | `dangerouslySetInnerHTML` | ❌ | No | 0 archivos |
| 110 | `startTransition` / Concurrent Features | ❌ | No | 0 archivos, pese a estar en React 18 |
| 111 | SyntheticEvent | ➖ | — | Conceptual, interno de React |
| 112 | Render Props | ❌ | No evidente | El proyecto resuelve composición con HOC (`connect`) y props normales, no con este patrón |
| 113 | Automatic Batching (React 18) | ➖ | — | Viene gratis con React 18, no es algo que se "use" explícitamente |
