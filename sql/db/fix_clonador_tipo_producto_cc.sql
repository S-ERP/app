-- Fix: clonador ahora mapea key_cuenta_contable_* de tipo_producto
-- usando _respuesta_contabilidad->'mapeo'->'cuenta_contable' igual que modelo_cliente.
...
CREATE OR REPLACE FUNCTION public.clonador(
    ...
    _respuesta_crm          json DEFAULT NULL::json,
...
BEGIN
    ...
    FOR v_row IN
        ...
        FROM tipo_producto WHERE key_empresa = _key_empresa_from
    LOOP
        v_new_tp_key := md5(random()::text || clock_timestamp()::text);
        INSERT INTO tmp_tp_map VALUES (v_row.key, v_new_tp_key);

        v_reemplazos := jsonb_build_object('key', v_new_tp_key, 'key_empresa', _key_empresa_to);
        ...
        IF _respuesta_contabilidad IS NOT NULL THEN
            ...
            END IF;
            v_new_cc_inv_key_map := _respuesta_contabilidad->'mapeo'->'cuenta_contable'->>v_row.key_cuenta_contable_ganancia;
            IF v_new_cc_inv_key_map IS NOT NULL THEN
                v_reemplazos := v_reemplazos || jsonb_build_object('key_cuenta_contable_ganancia', v_new_cc_inv_key_map);
            END IF;
            v_new_cc_inv_key_map := _respuesta_contabilidad->'mapeo'->'cuenta_contable'->>v_row.key_cuenta_contable_costo;
            IF v_new_cc_inv_key_map IS NOT NULL THEN
                v_reemplazos := v_reemplazos || jsonb_build_object('key_cuenta_contable_costo', v_new_cc_inv_key_map);
            END IF;
            v_new_cc_inv_key_map := _respuesta_contabilidad->'mapeo'->'cuenta_contable'->>v_row.key_cuenta_contable_depreciacion_activo;
            IF v_new_cc_inv_key_map IS NOT NULL THEN
                v_reemplazos := v_reemplazos || jsonb_build_object('key_cuenta_contable_depreciacion_activo', v_new_cc_inv_key_map);
            END IF;
            v_new_cc_inv_key_map := _respuesta_contabilidad->'mapeo'->'cuenta_contable'->>v_row.key_cuenta_contable_depreciacion_gasto;
            IF v_new_cc_inv_key_map IS NOT NULL THEN
                v_reemplazos := v_reemplazos || jsonb_build_object('key_cuenta_contable_depreciacion_gasto', v_new_cc_inv_key_map);
            END IF;
        END IF;

        PERFORM public.clonar_tabla('tipo_producto', 'key', v_row.key, v_reemplazos);
...
ALTER FUNCTION public.clonador(
    character varying, character varying, json, json, json
) OWNER TO postgres;
