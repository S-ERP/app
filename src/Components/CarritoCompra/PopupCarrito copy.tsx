...
import { SGradient, SInput, SMath, SNotification, SPopup, SText, STheme, SView } from "servisofts-component";
import MDL from "../../MDL";
            ...
            <SView col={"xs-12"} height >
                ...
                <SView row style={{ paddingHorizontal: 10, paddingVertical: 6, alignItems: "center", backgroundColor: colorCompra + "1c", borderBottomWidth: 1, borderBottomColor: colorCompra + "40", }}>
                    ...
                    <SText fontSize={UI.font.small} bold color={colorCompra}>
                        {"Sub: "}{selectedMoneda?.observacion ?? "Bs"}{" "}{SMath.formatMoney(MDL.carrito.carrito_compra?.monto_total || 0, 5)}
                    </SText>
                </SView>
                ...
                <SView style={{
                    ...
                    <SView row style={{ justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                        ...
                        <SText fontSize={UI.font.title} bold color={colorCompra}>
                            {selectedMoneda?.observacion ?? "Bs"}{" "}{SMath.formatMoney(MDL.carrito.carrito_compra?.monto_total || 0, 5)}
                        </SText>
                    </SView>
...
const ItemComp = ({ item, moneda }: { item: any; moneda: any }) => {
    ...
    const [precioStr, setPrecioStr] = React.useState(() => (calcularPrecio() ?? 0).toFixed(5));
    const [imgError, setImgError] = React.useState(false);
    const inputPrecioRef = React.useRef<SInput2Class>(null);
    React.useEffect(() => {
        const p = calcularPrecio();
        setPrecio(p);
        const str = (p ?? 0).toFixed(5);
        setPrecioStr(str);
    ...
    const precioFormateado = (precio ?? 0).toFixed(5);
    const subtotalStr = SMath.formatMoney(precio * (item.cantidad ?? 0), 5);
                ...
                <SView style={{ position: "relative" }}>
                    <SView style={{
                        ...
                        {imgError ? (
                            ...
                            // <SText fontSize={10} color={STheme.color.lightGray}>{(modelo?.descripcion?.trim()?.charAt(0) || "?").toUpperCase()}</SText>
                        ) : (
                            <Image
                                source={{ uri: (SSocket.api as any).inventario + "modelo/" + (modelo?.key ?? "") }}
