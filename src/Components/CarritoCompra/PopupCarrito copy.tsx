...
export default class PopupCarrito extends React.Component<PopupCarritoProps> {
    ...
    async cargarMonedas() {
        try {
            ...
            const selectedMoneda = monedaDefault ?? this.state.selectedMoneda ?? monedas[0] ?? null;
            if (selectedMoneda) {
    ...
    render() {
        ...
        return <SView col={"xs-12"} height>
            ...
            <SView padding={8}>
                ...
                {!selectedMoneda ? <SText color={STheme.color.danger} fontSize={11} style={{ textAlign: "right" }}>
                    Seleccione una moneda para continuar.
                </SText> : null}
            </SView>
            <SHr h={1} color={STheme.color.card} />
            <SView padding={8}>
