...
export default function PizarraMiniMapa({ children, style }: { children?: React.ReactNode, style?: ViewStyle, }) {
    ...
    const panGesture: any = Gesture.Pan().minVelocity(0.1)
        .onBegin((event) => {
            const scale1 = size / pizarra.size.value;
            const positionPizarrax = (pizarra.size.value / 2) - (event.x / scale1)
            const positionPizarray = (pizarra.size.value / 2) - (event.y / scale1)
            console.log(event)
