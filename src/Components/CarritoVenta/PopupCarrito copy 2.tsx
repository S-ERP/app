...
const ItemCompBase = ({ item, moneda }: { item: any; moneda: any }) => {
    ...
    }, [moneda, item.modelo.precio_venta, item.modelo.venta_moneda]);
    ...
    const precioFormateado = React.useMemo(() => (precio ?? 0).toFixed(2), [precio]);
    const puedeEditarPrecio = React.useMemo(() => MDL.rolesPermisos.getPermiso({ url: "/empresa/punto_venta", permiso: "carrito_editar_precio" }), []);
    const subtotalStr = React.useMemo(() => SMath.formatMoney(precio * item.cantidad), [precio, item.cantidad]);
    const precioDigits = React.useMemo(() => puedeEditarPrecio ? precioStr.replace(/[^0-9]/g, '').length : precioFormateado.replace(/[^0-9]/g, '').length, [puedeEditarPrecio, precioStr, precioFormateado]);
    const subtotalLargo = React.useMemo(() => puedeEditarPrecio ? precioDigits >= 10 : precioDigits >= 4, [puedeEditarPrecio, precioDigits]);
    const precioValido = React.useMemo(() => Number(precioFormateado) > 0, [precioFormateado]);
    return (
        <SView style={{
...
const ItemComp = React.memo(ItemCompBase, (prev, next) => {
    return prev.item === next.item && prev.moneda === next.moneda;
});
