...
class ListaIngredientes extends React.Component<{ item: any }> {
    mounted = true;
    state: { isOpen: boolean; ingredientes: any[] | null; nombresGrupo: Record<string, string>; visible: number } = {
        isOpen: true,
    ...
    render() {
        const { item } = this.props;
        const { isOpen, ingredientes, nombresGrupo, visible } = this.state;
...
const SuscripcionItem = React.memo(({ index, item, suscriptor, clientes, loadingClientes }: any) => {
    ...
    if (optionsRef.current.length !== clientes.length) {
        optionsRef.current = clientes.length > 0 ? clientes.map((c: any) => {
