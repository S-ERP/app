...
// Caché compartido de MDL.crm.cliente.getAll(): ListaSuscripciones se monta una vez por
// cada ítem del carrito con cupos de suscripción, y sin esto cada instancia dispara su propio
// fetch idéntico (N ítems = N llamadas redundantes al mismo listado completo de clientes).
let _clientesCachePromise: Promise<any[]> | null = null;
export const getClientesCached = (): Promise<any[]> => {
    if (!_clientesCachePromise) {
        _clientesCachePromise = MDL.crm.cliente.getAll()
            .then((resp: any) => (Array.isArray(resp) ? resp : Object.values(resp || {}).filter((c: any) => !!c)))
            .catch((err: any) => { _clientesCachePromise = null; throw err; });
    }
