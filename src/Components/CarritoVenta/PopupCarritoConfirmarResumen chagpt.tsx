...
import { SHr, SImage, SInput, SMath, SNavigation, SNotification, SPopup, SText, STheme, SView } from "servisofts-component";
import MDL from "../../MDL";
...
type PopupCarritoConfirmarResumenState = {
    almacen: any,
    moneda: any,
    factura: boolean,
    razon_social: string,
...
export default class PopupCarritoConfirmarResumen extends React.Component<PopupCarritoConfirmarResumenProps, PopupCarritoConfirmarResumenState> {
    ...
    state: PopupCarritoConfirmarResumenState = {
        almacen: null,
        moneda: null,
        factura: false,
        razon_social: "",
    ...
    async componentDidMount() {
        try {
            ...
            // Cargar descuentos
            const descuentosResp = await SSocket.sendPromise({
                service: "compra_venta",
                component: "descuento",
                type: "getAll",
                key_empresa: MDL.empresa?.select?.key
            });
            const descuentos = Object.values(descuentosResp.data);
            this.setState({ descuentos });

            // Escuchar cambios de moneda
            this.eventoMoneda = MDL.compra_venta.addEventListener("moneda_seleccionada", (moneda) => {
                this.setState({ moneda });
            });
        } catch (e) {
            console.error("Error cargando clientes o descuentos", e);
        }
    }

    componentWillUnmount() {
        if (this.eventoMoneda) {
            MDL.compra_venta.removeEventListener(this.eventoMoneda);
        }
    }
    ...
    handleSubmit = async (tipos_pago: any, key_moneda: string, cliente: any, factura: boolean, almacen_: any, porcentajeDescuento: any, descuentoSeleccionado: any) => {
        try {
            ...
            if (!almacen) throw "Debe seleccionar un almacen";
    ...
    render() {
        const { montoMaximo, porcentajeDescuento, solo_para_caja, cliente, factura, almacen, subtotal, descuentoSeleccionado } = this.props;
        // Usar moneda del state y proteger contra null
        const monedaActual = this.state.moneda || { observacion: this.state.moneda?.observacion, tipo_cambio: 1 };
        ...
        return <SView col={"xs-12"} height>
            ...
            <SView flex>
                <SView padding={8}>
                    <SView row col={"xs-12"}>
                        ...
                        <SView col={"xs-12"} border={STheme.color.card} style={{ borderRadius: 8, padding: 8, borderWidth: 2 }} height={100}>
                            <SView col={"xs-12"} row style={{ justifyContent: "space-between", marginBottom: 4 }}>
                                ...
                                <SText fontSize={13} bold color={STheme.color.text}>
                                    {monedaActual.observacion} {SMath.formatMoney(subtotal, 2)}
                                </SText>
                            </SView>
                            ...
                            <SView col={"xs-12"} row style={{ justifyContent: "space-between", marginBottom: 4, padding: 3 }}>
                                ...
                                <SText fontSize={18} bold color={STheme.color.text}>
                                    {monedaActual.observacion} {SMath.formatMoney(montoMaximo, 2)}
                                </SText>
                            </SView>
                        </SView>
                    </SView>
            ...
            <SView col={"xs-12"} row center height={40}>
                <SView padding={8} card onPress={() => this.handleOnPress()}>
