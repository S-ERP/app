...
const DraggableBox = ({ x = 0, y = 0, children, style, onChange, id }: DraggableBoxProps) => {
    const getBoardValues = useGetBoardValues();
    // const scale = getBoardValues ? getBoardValues().scale : useSharedValue(1)
    ...
    const onHandlerStateChange = (event: any) => {
        if (event.nativeEvent.state === State.END || event.nativeEvent.state === State.CANCELLED) {
            ...
            // if (getBoardValues) getBoardValues().getNodes
    ...
    return (
        <PanGestureHandler
            onGestureEvent={(event) => {
                if (getBoardValues) {
                    translateX.value = event.nativeEvent.translationX * (1 / getBoardValues().scale.value);
                    translateY.value = event.nativeEvent.translationY * (1 / getBoardValues().scale.value);
                } else {
