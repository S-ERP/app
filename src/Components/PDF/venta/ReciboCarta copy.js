...
const formatCurrency = (val, moneda) => `${toNumber(val).toFixed(2)} ${moneda?.observacion ?? 'Bs'}`;
        ...
        return (
            <SPDF.View style={{ width: "100%", height: 120, flexDirection: "row" }}>
                ...
                <SPDF.View style={{ flex: 3, height: "100%", justifyContent: "center", alignItems: "center" }}>
                    <SPDF.View style={{ width: "100%", height: "100%" }}>
                        {ReciboCarta.renderTotalesDetalle({ label: `SUBTOTAL ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(subtotal, data?.moneda) })}
                        {ReciboCarta.renderTotalesDetalle({ label: `DESCUENTO ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(descuento, data?.moneda) })}
                        {ReciboCarta.renderTotalesDetalle({ label: `TOTAL ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(total, data?.moneda) })}
                        {ReciboCarta.renderTotalesDetalle({ label: `MONTO GIFT CARD ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(montoGiftCard, data?.moneda) })}
                        {ReciboCarta.renderTotalesDetalle({ label: `MONTO A PAGAR ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(total, data?.moneda) })}
                        {ReciboCarta.renderTotalesDetalle({ label: `MONTO PAGADO ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(montoPagado, data?.moneda) })}
                        {ReciboCarta.renderTotalesDetalle({ label: `CAMBIO ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(cambio >= 0 ? cambio : 0, data?.moneda) })}
                        {ReciboCarta.renderTotalesDetalle({ label: `IMPORTE BASE CRÉDITO FISCAL ${data?.moneda?.observacion ?? 'Bs'}`, monto: formatCurrency(total, data?.moneda) })}
                    </SPDF.View>
                </SPDF.View>
            </SPDF.View>
        );
