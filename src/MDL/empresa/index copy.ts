...
import { SPopup, SStorage, STheme, SThread } from "servisofts-component";
...
export default class empresa extends MDLAbstract<EventListener> {
  ...
  setEmpresa(empresa: Empresa) {
    ...
    // }
    if (cambioDeEmpresa) {
      // Cada empresa tiene su propio set de monedas; invalidar el cache y la
      // moneda seleccionada previa para que se vuelva a elegir la moneda base
      // de la nueva empresa en vez de arrastrar la de la empresa anterior.
      this.monedas = undefined;
      MDL.compra_venta.setMonedaSeleccionada(null);
      MDL.compra_venta.dispatchEvent({ type: "moneda_seleccionada" });
      // El carrito (compra/venta) referencia productos/precios de la empresa
      // anterior; hay que vaciarlo y cerrar sus popups para no arrastrarlo.
