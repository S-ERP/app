...
export default class tipoMovimientoLead {
 ...
 async getAll() {
  const resp: any = await SSocket.sendPromise({ service: "crm", component: "tipo_movimiento_lead", type: "getAll", key_empresa: Model.empresa.Action.getKey() });
  return Object.values(resp.data);
 }

 async getByKey(key_empresa: String) {
  const resp: any = await SSocket.sendPromise({ service: "crm", component: "tipo_movimiento_lead", type: "getByKey", key_empresa: key_empresa })
  return resp.data;
 }
