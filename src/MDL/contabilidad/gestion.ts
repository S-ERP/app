...
export default class gestion {
    ...
    async getAll() {
        const resp: any = await SSocket.sendPromise({ service: "contabilidad", component: "gestion", type: "getAll", key_empresa: Model.empresa.Action.getKey() });
        return Object.values(resp.data ?? {});
    }

     async abrir(key_gestion: any) {
        let key_empresa = Model.empresa.Action.getKey();
        const resp: any = await SSocket.sendPromise({ service: "contabilidad", component: "gestion", type: "abrir", key_gestion: key_gestion, key_empresa: key_empresa });
        return resp.data;
    }

      async cerrar() {
        let key_empresa = Model.empresa.Action.getKey();
        const resp: any = await SSocket.sendPromise({ service: "contabilidad", component: "gestion", type: "cerrar", key_empresa: key_empresa });
        return resp.data;
    }
