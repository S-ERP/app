...
class recuperar extends Component {
    ...
    getForm() {
        return <SForm
            ...
            inputs={{
                telefono: { placeholder: "0000000", type: "phone", isRequired: true },
            }}
          
            onSubmit={(values) => {
                Model.usuario.Action.recuperarPass({ correo: (values.telefono + "").toLowerCase() }).then(resp => {
            ...
            <SPage title={"Recuperar Contraseña"}>
                <SView center>
                    <Container loading={!this.state.ready}>
                        ...
                        <SView height={40} />
                        <SText fontSize={16} color={STheme.color.text} center >Ingresa tu numero de WhatsApp y le enviaremos un mensaje para configurar o restablecer su nueva contraseña. </SText>
                        <SView height={40} />
                        {this.getForm()}
                        <SView height={16} />
