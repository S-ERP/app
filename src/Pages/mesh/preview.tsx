...
import { SGradient, SHr, SIcon, SList, SLoad, SNavigation, SNotification, SPage, SText, SView } from 'servisofts-component';
...
export default class Preview extends Component<any> {
    ...
    render() {
        return <SPage title={"world"} disableScroll>
            <SGradient colors={["#000000", "#232323"]} />
            <SThreeGLView

                onGestureEvent={event => {
                    const { translationX, translationY, velocityX, velocityY } = event.nativeEvent;
                    if (this.controls) this.controls.handleGesture(velocityX, velocityY);
                ...
                onCreate={({ gl, renderer, scene, camera }) => {
                    ...
                    new GLTFLoader().load(this.state.url, (glb) => {
                        SNotification.remove("load_model")
                        this.glb = glb;
                        if (this.actionBar) {
