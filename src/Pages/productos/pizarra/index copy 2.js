...
export default class pizarra extends React.Component {
    ...
    filtro_opacity3 = (obj) => {
        const { filtro, selectedStock, ingredientes, modelos } = this.state;
        ...
        if (filtro) {
            ...
            const textoCoincide = texto.includes(filtro.toLowerCase());

            if (obj.arrIngredientes) {
                const ingredientesRelacionados = ingredientes.filter(i => obj.arrIngredientes.includes(i.key));
                visible = visible && (textoCoincide || ingredientesRelacionados.some(i =>
                    JSON.stringify(i).toLowerCase().includes(filtro.toLowerCase())
                ));
            }

            if (obj.kr) {
                const modelosRelacionados = modelos.filter(m => obj.kr.includes(m.key));
                visible = visible && (textoCoincide || modelosRelacionados.some(m =>
                    JSON.stringify(m).toLowerCase().includes(filtro.toLowerCase())
                ));
            }

            if (!obj.arrIngredientes && !obj.kr) {
                visible = textoCoincide;
            }
        }

        // Filtro por stock: en vez de eliminar, solo atenuar
        if (selectedStock) {
            if (selectedStock.key === "con_stock" && obj.stock <= 0) visible = false;
            if (selectedStock.key === "sin_stock" && obj.stock > 0) visible = false;
        }

        return visible ? 1 : 0.3; // destacados completos o atenuados
    }

    filtro_opacity2 = (obj) => {
        const { filtro, ingredientes } = this.state;
        if (!filtro) return 1;

        const texto = JSON.stringify(obj).toLowerCase();
        const coincidencia = texto.includes(filtro.toLowerCase());

        // Para modelos: si algún ingrediente de la receta coincide con el filtro, mostrarlo también
        if (obj.arrIngredientes) {
            const ingredientesRelacionados = ingredientes.filter(i => obj.arrIngredientes.includes(i.key));
        ...
        // Para ingredientes: si coincide su descripción o su receta
        if (obj.kr) {
            const modelosRelacionados = this.state.modelos.filter(m => obj.kr.includes(m.key));
        ...
        return coincidencia ? 1 : 0.1;
    }

    // filtro_opacity = (obj) => {
    //     const { filtro, selectedSucursal, selectedAlmacen, selectedStock, ingredientes, modelos } = this.state;
    ...
    //         // Ingredientes relacionados con modelos
    //         if (obj.arrIngredientes) {
    //             const ingredientesRelacionados = ingredientes.filter(i => obj.arrIngredientes.includes(i.key));
    //             visible = visible && (textoCoincide || ingredientesRelacionados.some(i =>
    //                 JSON.stringify(i).toLowerCase().includes(filtro.toLowerCase())
    ...
    //         // Modelos relacionados con ingredientes
    //         if (obj.kr) {
    //             const modelosRelacionados = modelos.filter(m => obj.kr.includes(m.key));
    //             visible = visible && (textoCoincide || modelosRelacionados.some(m =>
    //                 JSON.stringify(m).toLowerCase().includes(filtro.toLowerCase())
    ...
    //     // 4. Aquí puedes agregar filtros de sucursal o almacén si aplican a los modelos/ingredientes
    //     if (selectedSucursal && obj.key_sucursal !== selectedSucursal.key) visible = false;
    //     if (selectedAlmacen && obj.key_almacen !== selectedAlmacen.key) visible = false;
    ...
    filtro_opacity = (obj) => {
        ...
        if (obj.kr) {
            ...
            // También mostrar si está conectado a al menos un modelo visible
            const conectadoVisible = modelosRelacionados.some(m => modelosVisiblesKeys.has(m.key));
            if (algunaCoincidencia || conectadoVisible) return 1;
        }
            ...
            .map(ingrediente => {
                ...
                return (
                    <PizarraNodo
                        ...
                        x={ipos.x}
                        y={ipos.y}
                        style={{ opacity: this.filtro_opacity(ingrediente) }} // atenúa según filtro
        ...
        return <SView style={{ position: "absolute", top: 10, left: 10, flexDirection: "row" }} >
            ...
            <SView width={200}>
                ...
                {/* <SInput
                    ...
                    onChangeText={e => { this.setState({ filtro: e }); this.loadData(); }} /> */}
            </SView>
            <SView width={8} />
            <SView width={150}>
                <FiltroSelector2
