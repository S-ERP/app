                ...
                <SForm2 ref={(ref: any) => this.form = ref} row
                    ...
                    inputs={{
                        ...
                        "precio_compra": {
                            ...
                            onChangeText: (text: string) => {
                                ...
                                if (decimalPart && decimalPart.length > 5) {
                                    const limitedText = integerPart + ',' + decimalPart.substring(0, 5);
                                    if (this.form) {
                                        this.form.setValues({ "precio_compra": limitedText });
                                    }
                                }
                            },
                            onSubmitEditing: () => {
                        ...
                        "precio_venta": {
                            ...
                            defaultValue: (!this.props.editObject?.precio_venta ? "" : SMath.formatMoney(parseFloat(this.props.editObject?.precio_venta ?? 0), 6)),
                            label: "Precio venta", placeholder: "0,00", type: "text",
                            onChangeText: (text: string) => {
                                const [integerPart, decimalPart] = text.split(',');
                                if (decimalPart && decimalPart.length > 5) {
                                    const limitedText = integerPart + ',' + decimalPart.substring(0, 5);
                                    if (this.form) {
                                        this.form.setValues({ "precio_venta": limitedText });
                                    }
                                }
                            },
                        },
