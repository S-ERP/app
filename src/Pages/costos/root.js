...
export default class root extends React.Component {
    ...
    async loadData() {
        try {
            ...
            const tipoCostoMap = Object.fromEntries(
                (tipo_costo || []).map(m => [m.key, m])
            );
            const almacenMap = Object.fromEntries(
                (almacen || []).map(m => [m.key, m])
            );
            const modeloMap = Object.fromEntries(
                (modelo || []).map(m => [m.key, m])
            );
            ...
            const dataMejorada = costos.map(e => {
                ...
                return {
                    ...
                    tipoCosto: tipoCostoMap[informacion?.key_tipo_costo] ?? null, // luego lo usamos
                    modelo: modeloMap[informacion?.key_modelo] ?? null, // luego lo usamos
                };
            });
            return dataMejorada;
        } catch (error) {
    ...
    renderModelo(modelo = {}) {
        ...
        return (
            <SView col="xs-12" center row>
                ...
                <SView width={5} />
                <SText flex numberOfLines={1} style={{ fontSize: 10 }}> {modelo?.descripcion || "almacen"} </SText>
            </SView>
        );
    }
