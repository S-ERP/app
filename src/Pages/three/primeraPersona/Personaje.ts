...
export default class Personaje extends THREE.Group {
    ...
    createBody(props: { Ammo: AmmoType, ammoWorld: any }) {
        ...
        this.body.setRollingFriction(0);
        this.body.setSpinningFriction(0);

        this.body.setFriction(0.5); // Ajustar la fricción si es necesario
    ...
    rotateObject(angle: number) {
        ...
        const currentTransform = new this.Ammo.btTransform();
        this.body.getMotionState().getWorldTransform(currentTransform);

        const currentPosition = currentTransform.getOrigin();
        // Aplicar la rotación al cuerpo de Ammo.js
        ...
        const newTransform = new this.Ammo.btTransform();
        newTransform.setIdentity();
        newTransform.setOrigin(currentPosition);
        newTransform.setRotation(ammoQuat);

        this.body.setWorldTransform(newTransform);
        this.body.getMotionState().setWorldTransform(newTransform);
    }
    ...
    applyJump() {
        ...
        //     return;
        // }
        const jumpImpulse = new this.Ammo.btVector3(0, 200, 0); // Ajusta este valor según sea necesario
        this.body.activate();
