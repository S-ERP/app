...
export default class Header extends Component {
  ...
  async loadData() {
    try {
      ...
      SNotification.send({ title: "Error HEADER", body: "No se pudo cargar las monedas.", type: "error", color: STheme.color.error, time: 5000 });
    }
  }
  ...
  async cargarSucursalGuardada() {
    try {
      ...
      SNotification.send({ title: "Error HEADER", body: "No se pudo cargar la sucursal guardada.", type: "error", color: STheme.color.error, time: 5000 });
    }
  }
  ...
  render() {
    ...
    if (loading) {
      return (
        <SView height={40} col={"xs-12"} center backgroundColor={STheme.color.background}>
    ...
    return (
      <SView col={"xs-12"} row center height={40} backgroundColor={STheme.color.background} style={{ borderBottomWidth: 1, borderColor: STheme.color.card, paddingHorizontal: 8 }}>
        ...
        <SView col={"xs-7 md-5 lg-3"} height row center style={{ justifyContent: "flex-end" }}>
          <SView row center style={{ borderRightColor: STheme.color.gray, borderRightWidth: 1, paddingRight: 8 }}>
            <SView center style={{ width: 28, height: 28, borderRadius: 16, marginRight: 8, overflow: "hidden" }}>
