...
import { SForm, SHr, SImage, SNotification, SPopup, SText, STheme, SView, Upload } from "servisofts-component"
...
export default class PopupUploadVoucher extends Component<Props> {
    ...
    state = {
        loading: false,
        uploadedVouchers: (this.props.data_vouchers ?? []) as VoucherFile[],
        fileValue: [] as any[],
    }

    componentWillUnmount() {
        this.state.uploadedVouchers.forEach((v) => {
            if (v.url && v.file) URL.revokeObjectURL(v.url)
        })
    }
    ...
    handleFileChange = (e: any) => {
        ...
        if (rechazados.length > 0) {
            SNotification.send({
                ...
                body: `Solo se permiten imágenes y PDF. ${rechazados.length} archivo(s) rechazado(s).`,
    ...
    removeVoucher = (index: number) => {
        ...
        if (removed[0]?.url && removed[0]?.file) URL.revokeObjectURL(removed[0].url)
    ...
    handleSubmit = async () => {
        if (!this.props.key_empresa || !this.props.key_caja_detalle) {
            SNotification.send({ title: "Error", body: "Faltan datos del detalle.", color: STheme.color.danger, time: 3000 })
            return
        }
        try {
            this.setState({ loading: true })
            const nuevosArchivos = this.state.uploadedVouchers.filter((v) => v.file)
            const archivosServidor = this.state.uploadedVouchers.filter((v) => !v.file)
            ...
            if (fallidos.length > 0) {
                SNotification.send({
                    ...
                    body: `${subidosOk.length} subido(s). Fallaron: ${fallidos.join(", ")}`,
    ...
    limpiarTodosLosVouchers = async () => {
        ...
        try {
            ...
            console.error("Error al limpiar vouchers:", error)
            SNotification.send({
                title: "Error",
                body: "No se pudo limpiar los comprobantes.",
                color: STheme.color.danger,
                time: 3000,
            })
        }
            ...
            <SView>
                    ...
                    <SView
                        ...
                        onPress={() => {
                            SPopup.confirm({
                                ...
                                message: "¿Estás seguro de eliminar todos los comprobantes? Esta acción no se puede deshacer.",
                                onPress: () => this.limpiarTodosLosVouchers(),
                            })
                        }}
                    >
                ...
                >
                    {uploadedVouchers.map((v, i) => {
                        const url = v.url ??
                            `${SSocket.api.root}empresa/${this.props.key_empresa}/voucher/${this.props.key_caja_detalle}/${encodeURIComponent(v.name)}?time=${new SDate().toString("yyyy-MM-ddThh:mm")}`
                        ...
                        return (
                            <SView key={i} style={{ width: 80, height: 90, marginRight: 10, borderRadius: 8, overflow: "hidden", borderWidth: 1, borderColor: STheme.color.card, backgroundColor: STheme.color.card, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4, }}>
                                ...
                                <SView style={{ position: "absolute", top: 4, right: 4, backgroundColor: "#dc3545", width: 24, height: 24, borderRadius: 12, justifyContent: "center", alignItems: "center", }} onPress={() => this.removeVoucher(i)}>
                                    <SText color="#fff" fontSize={12} bold> ✕ </SText>
                                </SView>
                                <SView style={{ position: "absolute", bottom: 4, right: 4, backgroundColor: "#28a745", width: 24, height: 24, borderRadius: 12, justifyContent: "center", alignItems: "center", }} onPress={() => Linking.openURL(url)}>
    ...
    renderFileEmptyMessage() {
        if (this.state.uploadedVouchers.length > 0) return null
        return (
            <SView
                height={180}
                ...
                onPress={() => {
                    if (this.form && (this.form as any).inputs?.file?.input?.click) {
                        (this.form as any).inputs.file.input.click()
                    } else {
            ...
            >
                <SView col={"xs-12"} row center flex>
                    <SView center>
                        <SView style={{ width: 56, height: 56, borderRadius: 28, backgroundColor: "#4786b1", alignItems: "center", justifyContent: "center", }}>
                            <SIconApp name="confirmar" height={24} />
                        </SView>
                        <SHr h={16} />
                        <SText center fontSize={14} bold color={STheme.color.text}>Arrastra tus archivos aquí</SText>
                        <SHr h={6} />
                        <SText center fontSize={13} color={STheme.color.lightGray}>o haz clic para seleccionar</SText>
                        <SHr h={10} />
                        <SText fontSize={11} color={STheme.color.lightGray}>PDF, JPG, PNG</SText>
                    </SView>
                </SView>
            </SView>
        )
