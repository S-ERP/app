...
export default class Abierta extends Component {
    ...
    componentDidMount() {
        // this.loadData();
        MDL.empresa.getFull().then((data) => {
            console.log(data);
            const suc = data.sucursales.find(suc => suc.puntos_venta.find(pv => pv.key == this.props.caja.key_punto_venta));
            const pv = suc.puntos_venta.find(pv => pv.key == this.props.caja.key_punto_venta);
            const pvtp = pv.punto_venta_tipo_pago
            console.log(suc, pv, pvtp);
        }).catch(e => {
            console.log(e);
        })
            ...
            <SView col={"xs-12"} center>
                ...
                <SView col={"xs-11 sm-10 md-8 lg-6"}>
                    ...
                    <SView row>
                        ...
                        <SText card padding={8} margin={4} onPress={this.componentDidMount.bind(this)}>Recargar</SText>
                        <SText card padding={8} margin={4}>Vender Productos</SText>
                        <SText card padding={8} margin={4}>Cobrar a Clientes</SText>
                        <SText card padding={8} margin={4}>Ingreso del Banco</SText>
                        <SText card padding={8} margin={4}>Otros Ingresos</SText>
                        <SText card padding={8} margin={4}>Comprar Productos</SText>
                        <SText card padding={8} margin={4}>Pagar a Proveedores</SText>
                        <SText card padding={8} margin={4}>Enviar al Banco</SText>
