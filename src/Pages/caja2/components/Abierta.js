...
export default class Abierta extends Component {
    ...
    async loadData() {
        try {
            ...
            let caja = this.getCaja();
            if (_key_caja) {
                const cajaCargada = await MDL.caja.getByKey(_key_caja);
                if (cajaCargada) {
                    caja = cajaCargada;
                    if (this._mounted) this.setState({ caja: cajaCargada });
                }
            }
    ...
    mensaje() {
        const key_user = this.getCaja()?.key_usuario;
        const usuario = Model.usuario.Action.getByKey(key_user);
        if (!usuario) return null;

        const keySucursal = this.getCaja()?.key_sucursal;
        const keyPuntoVenta = this.getCaja()?.key_punto_venta;
        const sucursal = this.state.empresa?.sucursales?.find(s => s.key === keySucursal);
        const puntoVenta = sucursal?.puntos_venta?.find(pv => pv.key === keyPuntoVenta);
        ...
        return <SView col={"xs-11 sm-10 md-8 lg-6"} center row   >
            ...
            <SView col={"xs-12"} padding={15} row style={{
                ...
                <SView row col={"xs-12 xl-3"} center style={{ borderRightWidth: 1, borderColor: STheme.color.card }}>
                    {this.getCaja()?.fecha_cierre ? <SView row center col={"xs-12"}>
                        ...
                        <SHr height={10} />
                        <SText col={"xs-12"} fontSize={12} center color={STheme.color.text}>{new SDate(this.getCaja().fecha_cierre).toString("DAY, dd de MONTH del yyyy a las hh:mm")}</SText>
                    </SView> :
                        <SView row center>
                ...
                <SView row col={"xs-12 xl-5"} padding={10}>
                    ...
                    <SView row width={10} />
                    <SText bold color={STheme.color.text} fontSize={16}>{new SDate(this.getCaja()?.fecha, "yyyy-MM-dd").toString("yyyy-MM-dd")}</SText>
                    <SHr />
                    <SText fontSize={12} color={STheme.color.lightGray}>Fecha de registro:</SText>
                    <SView row width={10} />
                    <SText color={STheme.color.text} fontSize={12}>{new SDate(this.getCaja()?.fecha_on, "yyyy-MM-dd").toString("yyyy-MM-dd hh:mm")}</SText>
                </SView>
                {!this.getCaja()?.fecha_cierre && <SView row col={"xs-12 xl-3"} center>
                ...
                <FlatList style={{ flex: 1, width: "100%", }}
                    ...
                    ListHeaderComponent={() => {
                        ...
                        return <SView col={"xs-12"} center>
                            ...
                            <SHr h={10} />
                            <TotalTipoPago key_punto_venta={this.getCaja()?.key_punto_venta} movimientos={this.state.movimientos} />
                            <SHr h={32} />
                            <SView col={"xs-11 sm-10 md-8 lg-6"} >
                                <SText bold fontSize={16}>Acciones Rápidas</SText>
                                <SHr h={10} />
                                <MenuAcciones caja={this.getCaja()} movimientos={this.state.movimientos} key_punto_venta={this.getCaja()?.key_punto_venta} />
                                <SHr h={32} />
                            </SView>
