...
export default class PopupPagoCuota extends Component {
    ...
    getCompraData = () => {
        ...
        const cuotasProcesadas = (this.cuotasCompras || []).map(c => {
            ...
            // const monto = 15;
            // // 5555555555555555555555555555555 si
            // const monto_base = 25;
            // const monto_total = 35;
            // const monto_total_base = 45;
            const monto = parseFloat(c.monto || 0);
            const monto_base = parseFloat(c.monto_base || 0);
    ...
    Item = ({ cuota }) => {
        ...
        return (
            // <Animated.View style={{ opacity: this.fadeAnim }}>
                <SView key={`cuota-${cuota.numero}`} col={'xs-12'} style={cuotaStyle} activeOpacity={0.7}>
                    ...
                    <SView row style={{ justifyContent: "space-between" }}>
                        ...
                        <SView col={"xs-8"} style={{ alignItems: 'flex-end' }} border={"transparent"}>
                            {cuota.moneda === monedaBaseLocal ? (
                                <SText fontSize={16} bold color={"pink"}>
                                    {monedaBaseLocal.observacion} {SMath.formatMoney(cuota.monto_base)}
                                </SText>
                            ) : (
                                <>
                                    <SText fontSize={16} bold color={"orange"}>
                                        {cuota.moneda.observacion} {SMath.formatMoney(cuota.monto)}
                                    </SText>
                                    <SText fontSize={16} bold color={"pink"}>
                                        M.Base {monedaBaseLocal.observacion} {SMath.formatMoney(cuota.monto_base)}
                                    </SText>
                                    {cuota.moneda !== monedaBaseLocal && (
                                        <SView style={{ position: "absolute", right: 0, top: 37 }}>
                                            <SText fontSize={16} bold color={STheme.color.text}>
                                                ⬆️ Monto sugerido actual {monedaBaseLocal.observacion} {SMath.formatMoney(cuota.monto * 10)}
                                            </SText>
                                        </SView>
                                    )}
                                </>
    ...
    render() {
        ...
        // arrrrrray.map(item => {
        //     (
        //         console.log("pinturaaa " + item.key)
        //     )
        // })
