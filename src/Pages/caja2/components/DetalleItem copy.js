...
export default class DetalleItem extends Component {
  ...
  iconotipoArchivo(documento_name = "", documento_type = "") {
    ...
    if (!empresaKey || !itemKey || !SSocket.api?.empresa) return null;

    const url = `${SSocket.api.empresa}empresa/${empresaKey}/voucher/${itemKey}/${documento_name}?time=${new SDate().toString("yyyy-MM-ddThh:mm")}`;
