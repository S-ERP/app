...
export default class DetalleItemVenta extends Component {
  ...
  render() {
    ...
    // const estado_pago = "Al Día"
    // if ((item.detalle?.cuotas_en_mora?.monto || 0) > 0) estado_pago = "En Mora";
    // if ((item.detalle?.cuotas?.total || 0) <= (item.detalle?.monto_amortizado || 0)) estado_pago = "Pagado";

    // const estadoInfo = statesTipo[estado_pago];
    const estadoInfo = statesTipo[
      (item.detalle?.cuotas?.total || 0) <= (item.detalle?.monto_amortizado || 0)
        ? "Pagado"
        : (item.detalle?.cuotas_en_mora?.monto || 0) > 0
          ? "En Mora"
          : "Al Día"
    ];
