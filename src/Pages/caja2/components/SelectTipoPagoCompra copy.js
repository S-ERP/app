...
export default class SelectTipoPagoCompra extends Component<SelectTipoPagoCompraProps> {
    ...
    renderItemTipoPago(item) {
        ...
        return <SView col={"xs-12"} style={{ marginBottom: 6 }}>
            <SView style={{
                borderWidth: select ? 2 : 1,
                borderColor: select ? headerColor : STheme.color.lightGray + "30",
                backgroundColor: select ? headerColor + "14" : STheme.color.card,
                borderRadius: 8,
                ...
                {select && <>
                    ...
                    <SView flex col={"xs-12"} center style={{ paddingVertical: 8, paddingRight: 8 }}>

                        <SView col={"xs-12"} withoutFeedback center>
                            ...
                            <SView width={"100%"} row center style={{
                                ...
                                <SView style={{ paddingRight: 4, paddingLeft: 4 }} row center onPress={() => {
                            ...
                            {(item?.moneda?.tipo_cambio != 1) &&
                                <SView width={"100%"} row center style={{ backgroundColor: STheme.color.card, borderWidth: 1, borderColor: STheme.color.lightGray + "40", borderRadius: 4, paddingHorizontal: 1, height: 32, justifyContent: "center", marginTop: 4 }}>
                                    <SText style={{ marginRight: 2 }} color={STheme.color.lightGray}>{this.moneda_base?.observacion}</SText>
                                    <SView flex row>
                                        <SInput2
                            ...
                            <SView width={"100%"} row center
                                style={{
                                    ...
                                    borderRadius: 4, height: 28, justifyContent: "center", marginTop: 4
                                }}
                            >
                                ...
                                <SView flex row center >
                                    <SInput2
                                        ...
                                        style={{ width: "100%", textAlign: "left", fontSize: 14, paddingRight: 4 }}
    ...
    calcularMontoInsertado() {
        return SMath.formatMoney(this.calcularMontoInsertadoNum(), 5);
    }
    ...
    render() {
        ...
        const statusMsg = nada ? "Seleccione un tipo de pago" : diffBase < -0.001 ? `Falta: ${obsBase} ${SMath.formatMoney(Math.abs(diffBase), 5)}` : diffBase > 0.001 ? `Vuelto: ${obsBase} ${SMath.formatMoney(diffBase, 5)}` : "✓ Monto exacto";
        const headerColor = this.headerColor;
                    ...
                    <>
                        <SView row style={{ padding: 8, paddingBottom: 4, gap: 8 }}>
                            <SView flex style={{ backgroundColor: STheme.color.card, borderRadius: 8, padding: 10, alignItems: "center", borderWidth: 1, borderColor: STheme.color.lightGray + "40" }}>
                                ...
                                <SText bold fontSize={16} color={STheme.color.text}>{this.moneda_base?.observacion ?? "Bs"}{" "}{SMath.formatMoney(MDL.contabilidad.round(Number(this.props.montoMaximo ?? 0)), 5)}</SText>
                                {this.moneda?.tipo != "base" && (
                                    <SText fontSize={11} color={STheme.color.text}>{obs}{" "}{SMath.formatMoney(montoAPagar, 5)}{" · "}{this.moneda?.descripcion}</SText>
                                )}
                            </SView>
                            <SView flex style={{ backgroundColor: statusColor + "18", borderRadius: 8, padding: 10, alignItems: "center", borderWidth: 2, borderColor: statusColor }}>
                                ...
                                <SText bold fontSize={16} color={statusColor}>{this.moneda_base?.observacion ?? "Bs"}{" "}{SMath.formatMoney(this.calcularMontoInsertadoBase(), 5)}</SText>
                                {this.moneda?.tipo != "base" && (
                                    < SText fontSize={11} color={statusColor}>{obs}{" "}{SMath.formatMoney(montoInsertadoNum, 5)}{" · "}{this.moneda?.descripcion}</SText>
                                )}
                            </SView>
                        </SView>
                        <SView style={{ paddingHorizontal: 8, paddingBottom: 8 }}>
                            <SView style={{
                                ...
                                {this.moneda?.tipo != "base" && Math.abs(diffBase) > 0.001 && (
                                    <SText fontSize={11} bold color={STheme.color.white}>{obs}{" "}{SMath.formatMoney(Math.abs(diff), 5)}{" · "}{this.moneda?.descripcion}</SText>
                                )}
                            </SView>
                        </SView>
                    </>
