    ...
    render() {
        ...
        return <SView col={"xs-12"} center>
            ...
            <SView row style={{ gap: 12 } as any}>
                <SView center padding={12} style={{ flex: 1, borderRadius: 8, borderWidth: 1, borderColor: STheme.color.lightGray + "60", backgroundColor: STheme.color.lightGray + "10" } as any} onPress={() => SPopup.close("Transferencia")}>
                    <SText bold color={STheme.color.text}>Cancelar</SText>
                </SView>
                <SView center padding={12} style={{ flex: 2, borderRadius: 8, backgroundColor: STheme.color.success, opacity: loading ? 0.6 : 1 } as any} onPress={() => { if (!loading) this.submit(); }}>
                    {loading ? <SLoad /> : <SText bold width={"100%"} color={STheme.color.text}>Confirmar</SText>}
                </SView>
            </SView>
        </SView>
    }
