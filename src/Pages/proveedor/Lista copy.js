        ...
        return (
            ...
            >
                ...
                <DinamicTable.Col key="tipo_cliente" label="Tipo proveedor"
                    ...
                    customComponent={e => {
                        // La librería fija la altura de fila en 30px (no respeta el `height`
                        // del Col), así que los tags no pueden envolver a varias líneas o se
                        // salen visualmente hacia la fila de abajo. Se muestra 1 tag + "+N".
                            ...
                            <>
                                {visibles.map(tc => (
                                    <SView key={tc.key} style={{
                                        ...
                                        <SView style={{ width: 8, height: 8, borderRadius: 100, backgroundColor: tc.color ?? STheme.colorFromText(tc.titulo) }} />
                                        <SText fontSize={10} numberOfLines={1} style={{ flexShrink: 1 }}>{tc.titulo}</SText>
                                    </SView>
                                ))}
                                {restantes > 0 && (
                                    <SView style={{
