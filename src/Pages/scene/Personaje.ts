...
export default class Personaje extends THREE.Group {
    ...
    createBody(props: { Ammo: AmmoType, ammoWorld: any, position?: any, rotation?: any }) {
        ...
        // Ajustar fricción
        this.body.setFriction(1.0); // Incrementar la fricción general
        this.body.setRollingFriction(1.0); // Incrementar la fricción de rodadura
        this.body.setSpinningFriction(1.0); // Incrementar la fricción de giro
        ...
        if (action) {
            ...
            if (this.onPlay != name) {
                ...
                if (currentAction) {
                    ...
                    // Suavizar la transición desde la acción actual a la nueva acción
    ...
    async update(props: { delta: number, Ammo: AmmoType }) {
        ...
        // Comparar las posiciones y rotaciones actuales con las almacenadas anteriormente
                ...
                if (speed > 0.2) {
                    if (Math.abs(movementDirection.x) > Math.abs(movementDirection.z)) {
                        if (movementDirection.x > 0) {
                            // Reproducir animación de movimiento hacia la derecha
                            this.playAnimation('walkright');
                        } else {
                            // Reproducir animación de movimiento hacia la izquierda
                            this.playAnimation('walkleft');
                        }
                    } else {
                        if (movementDirection.z < 0) {
                            // Reproducir animación de movimiento hacia adelante
                            this.playAnimation('walk');
                        } else {
                            // Reproducir animación de movimiento hacia atrás
                            this.playAnimation('walkback');
                        }
                    }
                } else if (this.onPlay != "idle" && speed < 0.01) {
