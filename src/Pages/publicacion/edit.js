...
class index extends Component {
    ...
    componentDidMount() {
        ...
        SSocket.sendPromise({
            ...
            const publicacion = e.data[this.pk];
            if (!publicacion) {
                SPopup.alert("Publicación no encontrada");
                SNavigation.goBack();
                return;
            }
            this.setState({
            ...
            console.error(e);
            SPopup.alert("Error al cargar la publicación");
            SNavigation.goBack();
        })
            ...
            <SPage title={"Nueva publicación"}
                ...
                < Container >
                    ...
                    <SView col={"xs-12"} colSquare style={{ backgroundColor: STheme.color.card, borderRadius: 8, overflow: "hidden" }} center>
                        <SInput ref={r => this.r_image = r}
                            defaultValue={SSocket.api.root + "publicacion/" + this.pk}
                            type={"image"} style={{
                                width: "100%",
                                height: "100%",
                                zIndex: 99
                    ...
                    <SHr />
                    <SInput ref={r => this.r_descripcion = r}
                        defaultValue={this.state?.data?.descripcion}
                        type='textArea' placeholder={"Escribe un pie de foto"} style={{
