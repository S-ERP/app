...
export default class FormRegistroTipoCliente extends Component<FormRegistroType & { defaultData?: any }> {
    static open(props: FormRegistroType) {
        SPopup.open({
            ...
            content: <SView backgroundColor={STheme.color.background} style={{ borderRadius: 8, maxWidth: 300 }} padding={16} withoutFeedback col={"xs-11"}>
                <FormRegistroTipoCliente {...props} onRegister={(e) => {
    ...
    render() {
        ...
        return <SView center>
            <SText bold>{defaultData ? "Editar Tipo Cliente" : "Crear Tipo Cliente"}</SText>

            <SForm
                ref={(ref: any) => this.form = ref}
                inputs={{
                    "titulo": {
                        label: "Titulo ", autoFocus: true, required: true, defaultValue: defaultData?.titulo, onSubmitEditing: () => {
                            if (this.form) this.form.focus("description");
                        }
                    },
                    "descripcion": {
                        label: "Descripcion", required: true, defaultValue: defaultData?.descripcion, type: "textArea",
                    ...
                    "color": {
                        label: "Color", defaultValue: defaultData?.color ?? STheme.color.text, type: "color",
                        placeholder:"#ffffff",
                        onSubmitEditing: () => {
                            if (this.form) this.form.submit();
                        }
                ...
                onSubmit={(e: any) => {
                    ...
                    const prom = data?.key ? MDL.crm.tipoCliente.editar(data) : MDL.crm.tipoCliente.registrar(data);
