    ...
    render() {
        return <SForm
            ...
            inputs={{
                ...
                departamento: {
                    label: "Departamento", col: "xs-4.6", required: true,
                    type: "select2",
                    options: MDL.crm.paises[0].departamentos.map(a => {
                        return a.nombre
                    }),
                    value: this.props.mdl_clienteProyecto_cliente?.departamento ?? "", onChangeText: e => {
                ...
                provincia: {
                    ...
                    type: "select2",
                    options: MDL.crm.paises[0].departamentos.find(a => a.nombre == this.props.mdl_clienteProyecto_cliente?.departamento)?.provincias.map(a => {
                        return a.nombre
                    }) ?? [],
                    value: this.props.mdl_clienteProyecto_cliente?.provincia ?? "", onChangeText: e => this.handleClienteChange("provincia", e)
                },
