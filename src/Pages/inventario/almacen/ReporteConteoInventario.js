...
import { SPage, SView, SText, SHr, SImage, STheme, SNotification, SPopup, SDate, SMath, SNavigation } from 'servisofts-component';
...
export default class ReporteConteoInventario extends Component {
    ...
    async loadInitialData() {
        try {
            let { fecha_inicio, fecha_fin, selectedStock } = this.state;

            const api = await MDL.inventario.getAll_reporte_conteo_inventario_detallado(fecha_inicio, fecha_fin);
            ...
            // Filtrado de stock si aplica
            let filteredData = api;
            // if (selectedStock) {
            //     if (selectedStock.key === "con_stock") {
            //         filteredData = api.filter(p => p.total_excedente > 0 || p.total_perdida > 0);
    ...
    mostrarTabla() {
        return (
            <DinamicTable
                ...
                //                     if (!fecha) return SNotification.send({
                ...
                //                         message: `El inventario Nro. ${e.row?.key_conteo} será eliminado.`,
                //                         onPress: async () => {
                //                             try {
                //                                 await MDL.inventario.anular_cardex(e.row?.key_conteo);
                //                                 SNotification.send({
                //                                     key: `anulado_${e.row?.key_conteo}`,
                //                                     title: "✅ Inventario Anulado",
                //                                     body: `El inventario Nro. ${e.row?.key_conteo} fue anulado.`,
                //                                     time: 5000,
            ...
            >
                            ...
                            <SView col={"xs-12"} row center  >
                                <SView style={{ width: 38 }} >
                                    <SView style={{ width: 24, height: 24, borderRadius: 100, overflow: "hidden", backgroundColor: STheme.color.card + "66" }}>
                                        <SImage src={`${SSocket.api.empresa}sucursal/${e.row?.key_almacen}`} style={{ resizeMode: "cover" }} />
                                    </SView>
                                </SView>
                ...
                <DinamicTable.Col key="total_perdida" label="T. Pérdidas" center width={80} data={(e) => e.row?.total_perdida || 0} cellStyle={{ backgroundColor: STheme.color.danger + "33" }}
                    customComponent={e => <SText style={e.textStyle} center bold color={(e.row?.total_perdida >= 1) ? STheme.color.text : STheme.color.lightGray}>{e.row?.total_perdida ? e.row?.total_perdida : null}</SText>}
                />

                {/* <DinamicTable.Col key="total_perdida" label="T. Pérdidas" width={80} center data={(e) => e.row?.total_perdida || 0} /> */}
                ...
                {/* <DinamicTable.Col key="total_excedente" label="T. Excedente" width={80} data={(e) => e.row?.total_excedente || 0} />
                <DinamicTable.Col key="estado" label="Estado" width={120} data={e => e.row?.fecha_confirmacion ? "CONFIRMADO" : "PENDIENTE"}
                    customComponent={e => <SView width={80} center style={{ backgroundColor: this.colorEstado(e.row?.fecha_confirmacion ? "CONFIRMADO" : "PENDIENTE") + "60", borderRadius: 4 }}>
                        <SText fontSize={11}>{e.row?.fecha_confirmacion ? "CONFIRMADO" : "PENDIENTE"}</SText>
                    </SView>} /> */}
        ...
        return (
            <SPage title="Reporte de Conteo de Inventario" disableScroll>
                ...
                {/* <SView row style={{ gap: 16, flexWrap: "wrap", paddingHorizontal: 4 }}>
                    <SText fontSize={13} color={STheme.color.lightGray}> Fecha: <SText fontSize={13} bold color={STheme.color.text}>{this.state.fecha_inicio || "Todos"}    {this.state.fecha_fin || "Todos"} </SText> </SText>
                </SView> */}
                <SHr height={8} />
                {this.mostrarTabla()}
            </SPage>
