...
export default class reporteMisCajas extends Component {
	constructor(props) {
		super(props);
		const hoy = new Date();
	...
	renderTabla() {
		console.log(MDL.usuario.session?.key)
		return (
			<DinamicTable
				ref={ref => (this.DinamicTable = ref)}
				loadData={() => this.loadInitialData().then(data => {
					let filteredData = data.filter(e => e.key_usuario === MDL.usuario.session?.key);
				...
				loadInitialState={async () => ({
					...
					filters: [
						...
						// { col: "key_usuario", operator: "=", value: MDL.usuario.session?.key, type: "string" },
					],
	...
	render() {
		return (
			<SPage title="Reporte de mis Cajas" disableScroll>
				<SHr></SHr>
