...
import { SHr, SInput, SNavigation, SPage, SPopup, SText, STheme, SView, } from 'servisofts-component';
import { FlatList } from 'react-native'; import FloatButtom from '../Components/FloatButtom';
      ...
      content: (
        <SView col="xs-12" center style={{ backgroundColor: STheme.color.background, maxWidth: 340, borderRadius: 8, overflow: "hidden", }} withoutFeedback >
                  ...
                  <SView row>
                    <SView width={16} height={16} center style={{ borderWidth: 1, borderColor: STheme.color.text, borderRadius: 2, marginRight: 6, backgroundColor: this.selectedTags.includes(tag) ? "#1975fe" : "transparent", }} >
                      {this.selectedTags.includes(tag) && (
                        <SText color={STheme.color.text} bold fontSize={12} >✓</SText>
                      )} </SView>
                    <SView width={20} row border={"transparent"}>
                      <SView width={14} height={14} style={{ backgroundColor: tag.color, borderRadius: 15, marginRight: 9, }} />
                    </SView>
          ...
          <SView col="xs-12" center style={{ borderTopWidth: 1, borderColor: STheme.color.lightGray + "55", paddingVertical: 8, }} >
            <SText fontSize={12} color={STheme.color.text} onPress={() => { SNavigation.navigate("/tag"); SPopup.close("tag-popup"); }} > Editar etiquetas </SText>
          </SView>
          { }
        </SView>
      ),
        ...
        <SView col="xs-12" center padding={16}>
          ...
          <SView row center style={{ gap: 8, flexWrap: "wrap", marginBottom: 16, }} >
            {selectedSorted.length === 0 ? (
              <SText color="#fd0000ff">  Ninguna</SText>
            ) : (
              selectedSorted.map(tag => (
                <SView key={tag.key_modelo_tag} row center style={{ backgroundColor: tag.color, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, gap: 6, }} >
                  <SText color={STheme.color.text} fontSize={13}>{tag.nombre}</SText>
                  <SText color={STheme.color.text} fontSize={16} onPress={() => { this.selectedTags = this.selectedTags.filter(t => t !== tag); this.openTagPopup(); this.forceUpdate(); }} >
                    ×
                  </SText>
                </SView>
              ))
