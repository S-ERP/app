...
export default class ListaProductos extends Component {
  ...
  componentDidMount() {
    ...
    SSocket.sendPromise({
      ...
      this.setState({ key_categoria: "todos" })
      this.setState({ selected: "Todos" })
    ...
    return <SView col={"xs-12"} row center padding={15} style={{
      ...
      <SView width={120} style={{ alignItems: 'flex-end' }}>
        ...
        <SView width={70} height={70} style={{
          ...
          SPopup.open({
            ...
            content: <SView col={"xs-11 md-8 lg-6 xl-4"} colSquare center style={{
              ...
              <SImage src={SSocket.api.inventario + "producto/" + producto.key + "?time=" + new Date().getTime()} />
            </SView>,
          })
        }}>
          <SImage src={SSocket.api.inventario + "producto/" + producto.key + "?time=" + new Date().getTime()} style={{
            resizeMode: "cover"
          }} />
        </SView>
        <SHr h={4} />
        ...
        <SView col={"xs-7"} center>
          <Cantidad data={{ ...producto, key_empresa: this.props.key_empresa }}
  ...
  categoria = (categoria) => {
    ...
    return <SView height backgroundColor={STheme.color.background} center onPress={() => {
      ...
      <SView col={"xs-12"} row style={{
        ...
        borderRadius: 5,
        borderBottomWidth: 3,
        borderBottomColor: (this.state.selected == categoria.nombre) ? STheme.color.text : STheme.color.background
      }}>
        <SText bold fontSize={15} color={(this.state.selected == categoria.nombre) ? STheme.color.text : STheme.color.gray} >{this.Capitalize(categoria.nombre)}</SText>
      </SView>
      {/* <SHr height={8} /> */}
      {/* LISTA DE PRODUCTOS */}
      {/* <SList
