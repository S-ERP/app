...
export default class ListItem2 extends Component {
    ...
    buildName() {
        ...
        return SUtil.limitString(obj.name, ((this.props?.width/this.props.numColumns) - 20) / 7);
        // return obj.name
    }
    buildFecha() {
        const { obj, i, path } = this.props;
    ...
    renderContent() {
        ...
        return <>
            <SView col={"xs-12"} flex center padding={8}>
                {this.buildIcon()}
            </SView>
            <SView height={30} style={{
                ...
                <SView col={"xs-12"}>
                    <SText col={"xs-12"} clean row fontSize={12} color={(this?.props?.obj?.name ?? "").startsWith(".") ? STheme.color.gray : STheme.color.text}>{this.buildName()}</SText>
                </SView>
                {/* <SText clean fontSize={8} color={STheme.color.gray}>{"Modificado:"} {this.buildFecha()}</SText> */}
                {/* <SText clean fontSize={8} color={STheme.color.gray}>{this.humanReadableFileSize(this.props?.obj?.size) ?? "-"}</SText> */}
            </SView>
            <SView style={{
                position: "absolute",
                right: 0
            }} width={40} height={40} center padding={4} onPress={e => {
                MenuItem.open({ obj: this.props.obj, path: this.props.path, onEvent: this.props.onEvent })
                e.preventDefault()
            }}>
                <SIcon name='drive-menu' fill={STheme.color.gray} />
    ...
    render() {
        ...
        const nc = this.props.numColumns ?? 1;
        return <SView
            col={"xs-" + (12 / nc)}
            colSquare
            style={{
